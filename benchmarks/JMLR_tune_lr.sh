...
docker run -it --rm --gpus '"device=6"' -v $(pwd):/fml -w /fml fedeval:v1 python trial.py -s FedSGD -c configs/quickstart -m local -i true -t lr -r $FED_EVAL_REPEAT -l $FED_EVAL_LOG_DIR -e simulate_fedsgd -d semantic140  # GPU6
...
docker run -it --rm --gpus '"device=7"' -v $(pwd):/fml -w /fml fedeval:v1 python trial.py -s LocalCentral -c configs/quickstart -m local -i true -t lr -r $FED_EVAL_REPEAT -l $FED_EVAL_LOG_DIR -e simulate_central -d semantic140 # GPU6
...
python trial.py -s LocalCentral -c configs/quickstart -m local -i true -t lr -r $FED_EVAL_REPEAT -l $FED_EVAL_LOG_DIR -e simulate_local -d semantic140  # WorkStation
...
python trial_by_cl.py -s FedSTC -r $FED_EVAL_REPEAT -l $FED_EVAL_LOG_DIR -c configs/quickstart -m without-docker -i true -t lr -e run -d semantic140
...
python trial.py -s FedSTC -r $FED_EVAL_REPEAT -l $FED_EVAL_LOG_DIR -c configs/quickstart -m local -i true -t lr -e run -d semantic140
...
python trial.py -s FedAvg -r $FED_EVAL_REPEAT -l $FED_EVAL_LOG_DIR -c configs/quickstart -m local -i true -t lr -e run -d semantic140  # GPU2
