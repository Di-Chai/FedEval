...
def upload_to_server(machines, local_dirs, file_type=('.py', '.yml')):
    files = []
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--log_path', '-p', help='log path, e.g., log/Server')
    args = args_parser.parse_args()

    if args.execute == 'log':
        if args.log_path is None:
            raise ValueError('Please provide log_dir')
        LogAnalysis(args.log_path).to_csv('_'.join(args.log_path.split('/')) + '.csv')
    else:
