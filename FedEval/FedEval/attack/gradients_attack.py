...
class AttackModel:
    ...
    def build_attack(self, gradient_names, batch_size, init_vars=True, max_to_keep=5, lr=None):
        ...
        with self.model.graph.as_default():
            ...
            for input_name in self.model.inputs_shape:
                input_tensor[input_name] = \
                    tf.Variable(
                        tf.random.normal(
                            [batch_size] + list(self.model.inputs_shape[input_name]),
                            dtype=tf.float32),
                        name=input_name
                    )
            ...
            for target_name in self.model.targets_shape:
                target_tensor_raw[target_name] = tf.Variable(tf.random.normal(
                    [batch_size] + list(self.model.targets_shape[target_name]),
                    dtype=tf.float32),
                    name=target_name)
    ...
    def attack(self, gradients, server_epochs, server_local_epoch, plot_epoch=200,
