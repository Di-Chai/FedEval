...
class Client(object):
    ...
    def __init__(self,
        ...
        self.weights_upload_url = 'http://' + server_host + ':%s' % server_port + '/upload/weights'
    ...
    def register_handles(self):
        ...
        def on_request_update(*args):
            ...
            self.logger.info('Finished Training, posting weights')

            obj_to_pickle_string(my_weights, os.path.join(self.log_dir, os.environ.get('CLIENT_ID') + '.pkl'))

            requests.post(self.weights_upload_url, files={
                'file': open(os.path.join(self.log_dir, os.environ.get('CLIENT_ID') + '.pkl'), 'rb')
            }, timeout=600)

            self.logger.info('Post finished')
