...
class Server(object):
    def __init__(self, server_config, model, host, port):
        ...
        self.socketio = SocketIO(self.app, max_http_buffer_size=10**12,
                                 async_handlers=True, ping_timeout=3600, ping_interval=3600)
        self.host = host
        self.port = port
    ...
    def register_handles(self):
        ...
        def handle_wake_up():
            ...
            self.logger.info('%s wakeup' % request.sid)
            emit('init')
            ...
            if data['round_number'] == self.current_round:
                ...
                if receive_all:
                    ...
                    for i in range(len(self.c_up)):
                        self.data_receive.append(asizeof.asizeof(self.c_up[i]))
                        self.c_up[i]['weights'] = pickle_string_to_obj(self.c_up[i]['weights'])
    ...
    def check_response(self, received, send_response, idol):
    ...
    def train_next_round(self, client_sids_selected):
        ...
        if self.current_round == 0:
            current_weights = obj_to_pickle_string(self.aggregator.current_weights, None)
            data_send = {
        ...
        for rid in client_sids_selected:
            ...
            counter += 1
            self.data_send.append(asizeof.asizeof(data_send))
    ...
    def evaluate(self):
        ...
        # time.sleep(0.01)
        self.data_send.append(asizeof.asizeof(data) * len(self.ready_client_sids))
        self.logger.info('Waiting resp from clients')
