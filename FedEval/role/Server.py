...
class Aggregator(object):
    ...
    def update_weights(self, client_weights, client_sizes):
        ...
        new_weights = client_weights[0]
        for c in range(1, len(client_weights)):
            for outer_key in new_weights:
                for key in new_weights[outer_key]:
                    if c == 1:
                        new_weights[outer_key][key] = new_weights[outer_key][key] * client_sizes[0] / total_size
                    new_weights[outer_key][key] += (client_weights[c][outer_key][key] * client_sizes[c] / total_size)
...
class Server(object):
    ...
    def train_next_round(self, client_sids_selected):
        ...
        if self.current_round == 0:
            ...
            data_size = asizeof.asizeof(data_send) + os.path.getsize(os.path.join(self.model_path,
                                                                                  self.latest_weight_filename))
        else:
            data_send = {
                'round_number': self.current_round
            }
    ...
    def evaluate(self):
        ...
        model_size = asizeof.asizeof(obj_to_pickle_string(self.aggregator.current_weights))
        self.c_eval = []
        data = {
            'current_weights_file': self.latest_weight_filename,
        ...
        emit('evaluate', data, broadcast=True, callback=self.response)
        self.data_send.append((asizeof.asizeof(data) + model_size) * len(self.ready_client_sids))
        self.logger.info('Waiting resp from clients')
