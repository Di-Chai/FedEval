...
class TFModel(FMLModel):
    ...
    def get_optimizer_weights(self):
        with self.graph.as_default():
            ...
            return {var.name: self.session.run(var) for var in tf.all_variables()
                    if str_hit(var.name, [self.optimizer.get_name()])}
    ...
    def get_weights(self, upload_name_filter=()):
        with self.graph.as_default():
            trainable_weights = {var.name: self.session.run(var) for var in tf.trainable_variables()
                                 if not str_hit(var.name, upload_name_filter)}
            optimizer_weights = self.get_optimizer_weights()
    ...
    def set_weights(self, value_dict):
        with self.graph.as_default():
            if 'trainable' in value_dict and 'optimizer' in value_dict:
                for variable in tf.all_variables():
                    if variable.name in value_dict['trainable']:
                        variable.load(value_dict['trainable'][variable.name], self.session)
                        print('Load', variable.name)
                    if variable.name in value_dict['optimizer']:
                        variable.load(value_dict['optimizer'][variable.name], self.session)
                        print('Load', variable.name)
            else:
                super(TFModel, self).set_weights(value_dict)
