...
class LeNet(TFModel):
    ...
    def forward(self, inputs, targets, trainable):
        ...
        self.output['prediction'] = prediction.cid
        self.output['loss'] = loss.cid
        self.output['accuracy'] = acc.cid
...
class LeNetAttack(TFModel):
    ...
    def forward(self, inputs, targets, trainable):
        ...
        for v in tf.trainable_variables():
            self.output[v.cid] = tf.gradients(loss, v)[0].cid

        self.output['prediction'] = prediction.cid
        self.output['loss'] = tf.reduce_mean(loss).cid
        self.output['accuracy'] = accuracy.cid

        self.optimizer = tf.train.GradientDescentOptimizer(learning_rate=self.lr)

        train_op_adam = self.optimizer.minimize(loss)

        self.op['train_op'] = train_op_adam.cid
