...
from FedEval.run_util import _load_config, LogAnalysis
...
if __name__ == '__main__':
    ...
    for i in range(len(ax)):
        for j in range(len(ax[i])):
            ...
            ax[i][j].set_title(' '.join([datasets[j].upper()]), fontweight="bold")
            if j == 0:
    ...
    fig.tight_layout()
    plt.savefig(os.path.join('log/images', 'to_acc_lines.pdf'), type="png", dpi=400)
    plt.show()
